...
async function loadHomeworkTable(filterSubject, filterStatus) {
  ...
  homework.forEach(hw => {
    ...
    tr.innerHTML = `
      ...
      <td><span class="badge badge-info">${hw.subject}</span></td>
      <td style="color:var(--text-secondary);font-size:13px;">${hw.chapter}</td>
      <td style="font-family:var(--font-mono);font-size:13px;">${formatDate(hw.date)}</td>
      <td>
...
async function loadAssignmentsTable(filterSubject, filterStatus) {
  ...
  assignments.forEach(asgn => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:500;">${asgn.title}</td>
      <td><span class="badge badge-info">${asgn.subject}</span></td>
      ...
      <td style="font-family:var(--font-mono);font-size:13px;">${formatDate(asgn.deadline)}</td>
      <td>
...
async function loadTestsTable() {
  ...
  tests.forEach(test => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td style="font-weight:500;">${test.test_name}</td>
      <td><span class="badge badge-info">${test.subject}</span></td>
      <td style="font-family:var(--font-mono);">${test.marks} / ${test.total}</td>
      <td><span class="${accColor}" style="font-family:var(--font-mono);font-weight:600;">${accuracy}%</span></td>
      <td style="font-family:var(--font-mono);font-size:13px;">${formatDate(test.date)}</td>
      <td>
        <div style="display:flex;gap:8px;">
          <div class="file-upload">
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        fill: true, tension: 0.4, pointBackgroundColor: '#4F8CFF', pointRadius: 6, pointHoverRadius: 8
      }]
    },
    options: {
      ...
      scales: {
        ...
        x: { ticks: { color: '#8A94A6', font: { size: 11 } }, grid: { display: false } }
      }
    }
  });
}
