...
async function loadChapterPage() {
  ...
  if (progressEl) {
    ...
    progressEl.innerHTML = `
      <div class="progress-bar-wrapper" style="height:6px;margin-bottom:8px;">
    ...
    item.innerHTML = `
      <div class="chapter-info">
        ...
        <div>
          ...
          <div style="font-size:11px;color:var(--text-muted);margin-top:2px;">7 parameters</div>
        </div>
      </div>
      <div class="chapter-progress">
        <div class="progress-bar-wrapper" style="width:120px;height:6px;">
...
async function loadChapterHomework(chapterId) {
  ...
  homework.forEach(hw => {
    const item = document.createElement('div');
    item.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:12px 16px;background:var(--bg-surface);border:1px solid var(--border-color);border-radius:var(--radius-sm);margin-bottom:8px;';
    item.innerHTML = `
      <div>
        <div style="font-size:14px;">${hw.title}</div>
...
async function loadChapterAssignments(chapterId) {
  ...
  assignments.forEach(asgn => {
    ...
    item.innerHTML = `
      <div>
        <div style="font-size:14px;">${asgn.title}</div>
